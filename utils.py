...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    ...
    if upsample:
        output = [np.zeros(shape)] * len(masks)
        for i, mask in enumerate(masks):
            output[i] = cv2.resize(mask, shape)  # CHW

        masks = np.array(output)
    return masks > 0.5
...
def getDeviceInfo(deviceId=None, debug=False, poe=True):
    """
    ...
    Returns:
        depthai.DeviceInfo.DeviceInfo: Object representing selected device info
    Raises:
    ...
    if not poe:
        deviceInfos = [di for di in deviceInfos if di.protocol != dai.X_LINK_TCP_IP.X_LINK_TCP_IP]
...
def toTensorResult(packet):
    ...
    for tensor in packet.getRaw().tensors:
        if tensor.dataType == dai.TensorInfo.DataType.INT:
            ...
            print("Unsupported tensor layer color_type: {}".format(tensor.dataType))
    return data
